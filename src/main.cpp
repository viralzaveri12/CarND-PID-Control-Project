...
int main()
{
  ...
  /* Steering angle PID */
  PID pid_steer;
  pid_steer.Init(0.2,0.002,7.8);
  
  /* Speed PID */
  PID pid_speed;
  pid_speed.Init(0.45,0.000,1.5);
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2')
    {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  /* Update error and calcuate steering value */
		  pid_steer.UpdateError(cte);
		  steer_value = -1 * pid_steer.TotalError();
		  
		  /* Update error and calcuate speed value */
		  pid_speed.UpdateError(fabs(steer_value));
		  speed_value = 0.6 - pid_speed.TotalError();
